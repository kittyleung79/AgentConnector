...
export class LLMProviderPresenter implements ILlmProviderPresenter {
  ...
  constructor(sqlitePresenter: ISQLitePresenter, configPresenter: ConfigPresenter) {
    ...
    try {
      ...
      while (needContinueConversation) {
        ...
        try {
          ...
          let currentEnabledMcpTools = enabledMcpTools
          if (typeof conversationId === 'string' && conversationId.trim().length > 0) {
            const conversation = await this.sqlitePresenter.getConversation(conversationId)
            currentEnabledMcpTools = conversation.settings.enabledMcpTools
          }
          const mcpTools =
            await presenter.mcpPresenter.getAllToolDefinitions(currentEnabledMcpTools)
