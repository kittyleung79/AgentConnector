...
/**
 * DeepLink 处理器类
 * 负责处理 doesaiagent:// 协议的链接
 * doesaiagent://start 唤起应用，进入到默认的新会话界面
 * doesaiagent://start?msg=你好 唤起应用，进入新会话界面，并且带上默认消息
 * doesaiagent://start?msg=你好&model=deepseek-chat 唤起应用，进入新会话界面，并且带上默认消息，model先进行完全匹配，选中第一个命中的。没有命中的就进行模糊匹配，只要包含这个字段的第一个返回，如果都没有就忽略用默认
 * doesaiagent://mcp/install?json=base64JSONData 通过json数据直接安装mcp
 */
export class DeeplinkPresenter implements IDeeplinkPresenter {
  ...
  init(): void {
    ...
    if (process.defaultApp) {
      if (process.argv.length >= 2) {
        app.setAsDefaultProtocolClient('doesaiagent', process.execPath, [
          path.resolve(process.argv[1])
        ])
      }
    } else {
      app.setAsDefaultProtocolClient('doesaiagent')
    }
    ...
    if (!gotTheLock) {
      ...
      app.on('second-instance', (_event, commandLine) => {
        ...
        if (process.platform === 'win32') {
          ...
          const deepLinkUrl = commandLine.find((arg) => arg.startsWith('doesaiagent://'))
          if (deepLinkUrl) {
            if (!app.isReady()) {
  ...
  async handleDeepLink(url: string): Promise<void> {
    ...
    try {
      ...
      if (urlObj.protocol !== 'doesaiagent:') {
