# DoesAIAgent User Guide

Welcome to DoesAIAgent! This guide will walk you through installing DoesAIAgent, configuring its features, and using it to interact with various Large Language Models (LLMs).
...
To get started with DoesAIAgent, download the latest version for your operating system from the [GitHub Releases page](https://github.com/ThinkInAIXYZ/doesaiagent/releases).
...
DoesAIAgent supports a wide range of LLM providers, both cloud-based and local.

1.  Launch the DoesAIAgent application.
2.  Click the **Settings icon** (often a gear or cogwheel symbol).
3.  Navigate to the **"Model Providers"** tab.

### Cloud LLM Providers

DoesAIAgent supports various cloud LLMs, including:
- DeepSeek
...
DoesAIAgent has integrated support for Ollama, allowing you to manage and use local models without command-line operations.
1.  In the "Model Providers" tab, select Ollama.
2.  DoesAIAgent allows you to:
...
DoesAIAgent supports a multi-window and multi-tab architecture, similar to a web browser. This allows for:
...
-   DoesAIAgent provides complete Markdown rendering for chat messages.
...
-   DoesAIAgent supports displaying multi-modal content within chats.
...
-   DoesAIAgent supports Artifacts rendering, which provides diverse ways to present results from LLMs, especially when using Tool Calling (MCP).
...
DoesAIAgent enhances LLM responses by integrating with search engines. This provides more accurate, timely, and verifiable information.
...
-   **Simulated Web Browsing**: DoesAIAgent can simulate user web browsing for mainstream search engines like Google, Bing, Baidu, and Sogou Official Accounts. This allows the LLM to "read" search engine results like a human.
-   **Custom Search Engines**: You can configure DoesAIAgent to use virtually any search engine, including internal corporate networks or specialized vertical domain search engines, by setting up a search assistant model.
...
DoesAIAgent features excellent Model Context Protocol (MCP) support, allowing LLMs to use tools and access external resources.
...
DoesAIAgent prioritizes user privacy and data security.

### Screen Projection Hiding

-   To prevent sensitive information from being accidentally displayed during screen sharing or projections, DoesAIAgent offers a screen projection hiding feature. This typically obscures the chat content when active.

### Network Proxies

-   You can configure DoesAIAgent to use network proxies, adding an extra layer of privacy and potentially bypassing network restrictions.

### Data Storage and Encryption

-   Chat data and configuration data have reserved encryption interfaces.
-   DoesAIAgent focuses on local data storage where possible to reduce the risk of information leakage.

## DeepLink Support

DoesAIAgent utilizes rich DeepLink support, which allows:
