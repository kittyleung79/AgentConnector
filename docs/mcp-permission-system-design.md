...
This document describes the redesigned permission request system for MCP (Model Context Protocol) tool calls in DoesAIAgent. The system provides a user-friendly way to request and manage permissions for tool executions while maintaining the conversational flow.
