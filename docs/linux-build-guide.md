# DoesAIAgent Linux 构建指南

## 功能概述

本文档描述了如何使用Docker在低版本glibc环境下构建DoesAIAgent应用程序。这种构建方式主要用于确保生成的二进制文件能在大多数Linux系统上运行。
...
```bash
# 构建Docker镜像
docker build -t doesaiagent-builder -f Dockerfile.build.linux .

# 运行构建
docker run --rm -v $(pwd):/app/dist doesaiagent-builder
```
